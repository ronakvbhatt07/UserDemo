...
namespace UserDemo.Controllers
{
    ...
    public class UserController : ApiController
    {
        ...
        {
            return entities.sp_InserUserDetails(user.name, user.age, user.salary, user.designation, user.DOB,user.last_name,user.location).ToString();
        }
        [HttpPut]
        public string Put(userDetail user)
        {
            return entities.sp_UpdateUserDetails(user.ID, user.name, user.age, user.salary, user.designation, user.DOB, user.last_name,user.location).ToString();
        }
    }
}
